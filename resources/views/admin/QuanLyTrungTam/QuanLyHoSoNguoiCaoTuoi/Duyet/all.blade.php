                            ...
                            @if (count($elderly) > 0)
                                @foreach ($elderly as $key => $usernd )
                                    <tr>
                                        ...
                                        <td><a href="{{route('user.edit',$usernd->id)}}">{{$usernd->hoTen}}</a></td>
                                        <td>{{date('d-m-Y', strtotime($usernd->ngaySinhNCC))}}</td>
                                        <td>{{$usernd->soDienThoaiNCC}}</td>
                                        <td>{{$usernd->CCCD_NCC}}</td>
                                        <td>{{date('d-m-Y', strtotime($usernd->ngayDangKyNCC))}}</td>
                                        <td class="text-center">
                                            <a href="{{route('browseelderly.detail',$usernd->id_nguoicaotuoi)}}" type="button" class="btn btn-info edit-elderly" style="border-radius: 7px"><i class="fa fa fa-info-circle"></i></a>
